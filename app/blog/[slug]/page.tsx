...
// ─── Fetch ALL posts from blob, parse JSON safely ────────────────────────────
export async function getAllPostsFromBlob() {
...
// ─── Find a single post by matching slug field inside JSON ───────────────────
// This is resilient regardless of how the blob filename was stored.
export async function getPostFromBlob(slug: string) {
  const all = await getAllPostsFromBlob();
...
export async function generateMetadata({ params }: Props) {
  ...
  const post = await getPostFromBlob(decodeURIComponent(slug));
  ...
  return (
    <div className="bg-[#F6F6F6] max-h-screen">
      ...
      <main className="pb-20 bg-white">
        {/* ── Hero / Cover ──────────────────────────────────────────────── */}
        <section className="relative w-full overflow-hidden mt-0 pt-20 bg-white">
        ...
        {/* ── Main Content ──────────────────────────────────────────────── */}
        <section className="max-w-4xl mx-auto pt-0 px-4 bg- sm:px-6 lg:px-8 py-16 md:py-24">
          <div className="grid grid-cols-1 mt-0 lg:grid-cols-4 gap-12">
            {/* Article */}
            <article className="lg:col-span-3">
              <div
                className="prose prose-sm sm:prose md:prose-lg max-w-none
                  ...
                  prose-h2:text-2xl sm:prose-h2:text-3xl prose-h3:text-xl sm:prose-h3:text-2xl
              ...
              <section className="mt-16 pt-12 border-t border-gray-200">
                <div className="bg-gradient-to-r from-[#14b8a558] to-[#dbebf3] rounded-2xl p-8 md:p-12 text-center">
                  <h3 className="text-2xl md:text-3xl font-bold text-[#1A1F2E] mb-3">
            ...
            <aside className="lg:col-span-1">
              <div className="sticky top-24 space-y-8">
                {relatedPosts.length > 0 && (
                  <div className="bg-white rounded-2xl p-6 px-2 shadow-sm">
